...
@section('title', 'Kelola Konten Halaman Kustom')
@section('page-title', 'Kelola Konten Halaman Kustom')
...
<div class="space-y-6">
  ...
  <section class="space-y-2">
    <a href="{{ route('admin.custom-page.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-700 bg-white border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-slate-50 hover:border-slate-300 transition">
      <span class="material-symbols-outlined text-[18px]">arrow_back</span>
      Kembali ke Halaman Kustom
    </a>
    <h2 class="text-2xl font-black text-gray-900">Konten Halaman: {{ $customPage->name }}</h2>
    <p class="text-sm text-slate-500">Kelola konten untuk halaman <span class="font-semibold">/{{ $customPage->slug }}</span>.</p>
  </section>

  <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
    <label for="modal-create-custom-card" class="group rounded-2xl border-2 border-dashed border-slate-300 bg-white p-6 min-h-[220px] flex flex-col items-center justify-center text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/40 transition">
      ...
      <article class="group relative rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm hover:shadow-xl transition">
        ...
        <div class="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
          <button type="button" class="w-10 h-10 rounded-full bg-white text-slate-700 flex items-center justify-center hover:text-amber-700 open-edit-modal"
            data-id="{{ $card->id }}"
            data-update-url="{{ route('admin.custom-card.update', [$customPage, $card]) }}"
            data-name="{{ $card->name }}"
            data-url="{{ $card->url }}"
          ...
          <button type="button" class="w-10 h-10 rounded-full bg-white text-red-600 flex items-center justify-center hover:bg-red-50 open-delete-modal"
            data-id="{{ $card->id }}"
            data-delete-url="{{ route('admin.custom-card.destroy', [$customPage, $card]) }}"
            data-name="{{ $card->name }}"
            title="Hapus">
  ...
  @if($cards->isEmpty())
    <section class="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">Belum ada konten di halaman ini.</section>
  @endif
</div>

<input type="checkbox" id="modal-create-custom-card" class="peer hidden">
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 opacity-0 pointer-events-none peer-checked:opacity-100 peer-checked:pointer-events-auto transition">
  <div class="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
      ...
      <label for="modal-create-custom-card" class="w-9 h-9 rounded-full hover:bg-slate-100 flex items-center justify-center cursor-pointer text-slate-500">✕</label>
    </div>
    <form action="{{ route('admin.custom-card.store', $customPage) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
      @csrf
      <div>
      ...
      <div class="grid grid-cols-2 gap-3 pt-2">
        <label for="modal-create-custom-card" class="h-11 rounded-xl border border-slate-300 flex items-center justify-center font-semibold text-slate-700 cursor-pointer">Batal</label>
...
<script>
  (function () {
    ...
    const createForm = document.querySelector("form[action='{{ route('admin.custom-card.store', $customPage) }}']");
